...
@router.get("/user/{user_id}", response_model=List[Keyword])
def get_keywords_by_user(user_id: int, db: Session = Depends(get_db)):
    keywords = crud_keywords.get_keywords_by_user_id(db, user_id)
    return keywords
